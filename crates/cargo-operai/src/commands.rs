...
//!
//! This module organizes all top-level commands available in the `cargo operai`
//! CLI. Each subcommand is implemented in its own module and provides a
//! distinct piece of functionality:
//!
//! - **`new`**: Scaffold a new Operai tool or workspace project from templates
//! - **`build`**: Compile an Operai project and generate embeddings for tool
//!   discovery
//! - **`serve`**: Start a gRPC server hosting Operai tools
//! - **`mcp`**: Run a Model Context Protocol (MCP) server for AI assistant
//!   integration
...
//! - An `*Args` struct implementing `clap::Args` for command-line argument
//!   parsing
//! - A `run` function (typically `async`) with signature `fn run(&Args) ->
//!   Result<()>>`
//!
