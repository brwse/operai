...
fn main() {
    ...
    if let Ok(contents) = std::fs::read_to_string(&workspace_toml_path)
        ...
        && let Some(workspace) = parsed.get("workspace").and_then(|w| w.as_table())
        && let Some(deps) = workspace.get("dependencies").and_then(|d| d.as_table())
    {
        if let Some(operai) = deps.get("operai")
            && let Some(version) = operai.as_table().and_then(|t| t.get("version"))
        ...
        if let Some(operai_build) = deps.get("operai-build")
            && let Some(version) = operai_build.as_table().and_then(|t| t.get("version"))
            && let Some(v) = version.as_str()
        {
